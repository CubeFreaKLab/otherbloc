...
The visual suite must finish before live services start: its CI web-server configuration intentionally refuses to reuse an existing Vite server. API integration requires `demo-otherbloc-test` and uses named databases/buckets unique to each test-file process; different project labels alone do not establish isolation. Live browser journeys use `demo-otherbloc` with six demo accounts and seven published readings. All these resources exist only inside that run's emulators. Actual business requests use the gateway and owner-service APIs; successful business responses in live E2E are not replaced by fixtures. E2E runs serially without retries. The separate visual suite uses explicit fixtures and retains its existing CI retry policy.
...
The final job summary records the verified revision and five public origins. Safe Render deploy IDs/statuses remain in the job log. If Google authentication/Hosting fails after Render succeeded, report that partial state and inspect it before a new dispatch; do not claim an atomic rollout. The [Firebase setup and controlled bootstrap](firebase-deployment.md) is a separate authorized operator step, never run implicitly in CI/CD. Complete the three-role browser journeys and Firebase business-persistence checks separately, including actual production cookie compatibility. Retain the Actions URL and exact deployed SHA only after the run exists.
