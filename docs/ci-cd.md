...
Production environment variables are the `RENDER_*` destinations in `deploy/.env.example`, `FIREBASE_PROJECT_ID`, `FIREBASE_HOSTING_ORIGIN`, `GOOGLE_WORKLOAD_IDENTITY_PROVIDER` and `GOOGLE_DEPLOY_SERVICE_ACCOUNT`. The Hosting origin must identify that project's default `https://<project>.web.app` site, not an arbitrary site or custom domain. `RENDER_API_KEY` is an environment secret exposed only to the Render step. `VITE_API_URL` and `VITE_GRAPHQL_URL` are derived from the gateway origin. Do not add backend JWT, Firebase runtime keys, seed passwords or private emulator settings to the release job.
