...
[Platform CI](ci-cd.md) adds a separate disposable-checkout workflow for visual tests, API integration and the complete live browser suite. Its `test:ci` stage refuses existing private configuration or occupied ports and must not be run over an established development environment. The [README](../README.md) now provides the clean-install sequence, three-role walkthroughs, test selection and release boundaries. Hosting/Render and real GitHub CI/CD still require their own evidence; technical documentation does not replace that acceptance.
