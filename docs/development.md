...
M7 verifies snapshot restoration and installation from a separate clean checkout: fresh dependencies/configuration, an empty emulator database, both seeds, 36 Node checks, 30 API checks and fourteen desktop user/content/editor/interactions journeys. Existing Node/npm, Java and the per-user Chromium cache were reused as prerequisites; no private environment file or database was inherited. Final performance/load verification, Hosting/Render and real CI/CD remain separate milestones. README remains intentionally empty until the final technical-documentation phase.
