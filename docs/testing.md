...
## Controlled demo bootstrap and actual emulator isolation — M7G6

Ten new Node contracts bring the total to 85. The separate cloud operator entry validates mode/stage, clean exact revision, explicit project confirmation, default bucket, owner credential format and four healthy revisions before the selected data stage. Its offline plan does not authenticate or call APIs. Local seeds keep their emulator-only guards and share only command orchestration, not backend runtime internals. See [Firebase setup and bootstrap](firebase-deployment.md).

A new real integration case creates six demo accounts and seven Storage-backed publications through owner seed methods and existing gateway operations, then repeats the seeds and compares unchanged account/publication hashes. The complete 31-case API suite passes in 69.85 seconds, including default-resource negative checks. Before/after fingerprints match all 2,395 documents in 13 development collections and all 267 Storage files (2,199,104 bytes); these are dated development-state counts, not required seed sizes. Normal local seed entry points also pass, preserving existing records. Two real desktop/mobile reading checks pass in 13.2 seconds and the desktop author/review/publication/archive journey passes in 28.5 seconds; both actual editor-theme captures were inspected. Lint/build pass. No new full 78-case E2E or cloud run is claimed.

The first seed test correctly refused existing fixed demo IDs. Trying another project ID still exposed the imported emulator's default database; read-only SDK and owner-REST diagnostics confirmed this behavior. Project labels alone did not establish the isolation described in earlier documentation. The test-only client adapter now supplies per-file random named Firestore databases and Storage buckets, keeps loopback/demo guards and performs scoped cleanup. Real writes/uploads are checked absent from default resources. Product factories, the deployed default database and authorization logic are unchanged. Direct-client rules are tested separately on the default emulator resources; implicit named databases are not evidence of deployed rules.

Failed diagnostics were retained. An initial whole-state fingerprint differed only in one pre-existing active session, updated during the measurement; all other collections and Storage already matched. A later complete 31-case run and before/after comparison match the entire development state. No data was reset to obtain the result. The actual cloud CLI refuses the original configured checkout before connecting; real IAM, provisioned resources, protected deployment and three-role cloud acceptance remain pending.
