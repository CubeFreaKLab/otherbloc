...
for (const url of ['http://127.0.0.1:3000/health', 'http://127.0.0.1:3001/health', 'http://127.0.0.1:3002/health', 'http://127.0.0.1:3003/health', 'http://127.0.0.1:5173']) {
  const response = await fetch(url, { signal: AbortSignal.timeout(5000) }).catch(() => null)
  if (!response?.ok) throw new Error('Start npm run dev and npm run emulators, then seed:users and seed:content, before test:e2e.')
}
