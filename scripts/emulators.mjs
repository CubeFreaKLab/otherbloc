...
const args = [cli]
if (mode === 'test') {
  args.push('emulators:exec', '--project', 'demo-otherbloc', '--only', 'firestore,storage', 'node --test --test-concurrency=1 tests/integration/*.test.js')
} else if (mode === 'export') {
...
const child = spawn(process.execPath, args, { cwd: root, stdio: 'inherit', env: { ...process.env, GCLOUD_PROJECT: 'demo-otherbloc' } })
