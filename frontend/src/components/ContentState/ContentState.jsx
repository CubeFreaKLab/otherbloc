...
export default function ContentState({ status = 'empty', message, onRetry }) {
  if (status === 'loading') {
    return (
  ...
  return (
    <div className={`content-state content-state--${status}`} role={status === 'error' ? 'alert' : 'status'}>
      ...
      <p>{copy}</p>
      {onRetry && <button type="button" className="secondary-button" onClick={onRetry}>Reintentar</button>}
    </div>
  )
}
