...
import { cloudinaryOptions, createCloudinaryBucket } from '../lib/cloudinary-bucket.js'
...
export function validateFirebaseEnvironment(source = process.env) {
  ...
  if (provider === 'cloudinary') cloudinaryOptions(source)
...
export function getFirebase() {
  ...
  return { app, db: getFirestore(app), bucket: config.provider === 'cloudinary' ? createCloudinaryBucket(process.env) : getStorage(app).bucket() }
}
