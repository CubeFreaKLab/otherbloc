...
test('creation is authenticated, author-owned and idempotent; drafts remain private even to moderation retries', async () => {
  ...
  ownedPublications.add(id)
  assert.equal((await http('POST', '/api/publications', { user: users.reader, body: { id } })).status, 201)
  ...
  assert.equal((await http('GET', '/api/publications/mine', { user: users.reader })).status, 200)
...
test('role changes and logout are revalidated by Users instead of trusting token role claims', async () => {
  ...
  const normal = { id: users.other.id, token: login.body.accessToken }
  const created = await create(normal)
  assert.equal(created.authorId, users.other.id)
  assert.equal((await http('PUT', url(draft.id), { user: normal, version: draft.version, body: editor(draft, { title: 'Sigo escribiendo con mi misma cuenta' }) })).status, 200)
  const suspended = await http('PATCH', '/api/users/' + users.other.id + '/permissions', { user: users.admin, body: { role: 'reader', status: 'suspended' } })
  assert.equal(suspended.status, 200)
  assert.equal((await http('GET', '/api/publications/mine', { user: normal })).status, 401)
