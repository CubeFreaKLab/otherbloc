...
test('real reading transitions share one decoded cover in both directions and both themes', async ({ page }, testInfo) => {
  ...
  for (const theme of ['light', 'dark']) {
    ...
    expect(forward.before.covers).toHaveLength(1)
    expect(forward.after.covers).toHaveLength(1)
    expect(forward.before.covers[0].loaded).toBe(true)
    expect(forward.after.covers[0].loaded).toBe(true)
    expect(forward.after.loading).toBe(false)
    expect(forward.animations.some((animation) => animation.target === '::view-transition-group(reading-cover)' && animation.duration === 320)).toBe(true)
    ...
    expect(back.after.covers).toHaveLength(1)
    expect(back.after.covers[0].loaded).toBe(true)
    expect(errors).toEqual([])
  }
})

test('main destinations fade, private parent-child navigation reverses, and theme retains its own snapshot', async ({ page }) => {
  await observeTransitions(page)
  await page.goto('/')
  await expect(page.locator('.featured-article')).toBeVisible()
  ...
  await page.getByRole('combobox', { name: 'Tema de color' }).selectOption('dark')
  await expect(page.locator('html')).not.toHaveAttribute('data-theme-transition')
  const theme = await page.evaluate(() => window.routeTransitions.at(-1))
  expect(theme.mode).toBe('theme')
  expect(theme.animations.some((animation) => animation.target === '::view-transition-group(route-content)')).toBe(false)
