...
test('the initial header leaves, then its compact bar slides back while scrolling downward and remains keyboard accessible', async ({ page }) => {
  await page.goto('/')
  const header = page.locator('.site-header')
  await expect(page.locator('.featured-article')).toBeVisible()
  await page.locator('main').focus()
  const mainTop = await page.locator('main').evaluate((node) => node.getBoundingClientRect().top + scrollY)
  await header.evaluate((node) => scrollTo({ top: node.offsetHeight + 60, behavior: 'instant' }))
  await expect(header).toHaveAttribute('data-mode', 'away')
  const bar = page.locator('.site-header__bar')
  await expect.poll(() => bar.evaluate((node) => node.getBoundingClientRect().bottom)).toBeLessThan(1)
  await page.evaluate(() => scrollTo({ top: 700, behavior: 'instant' }))
  await expect(header).toHaveAttribute('data-mode', 'docked')
  await expect.poll(() => bar.evaluate((node) => Math.abs(node.getBoundingClientRect().top))).toBeLessThan(1)
  expect(await page.locator('main').evaluate((node) => node.getBoundingClientRect().top + scrollY)).toBe(mainTop)
  await page.getByRole('button', { name: 'Abrir menú' }).click()
  await expect(page.getByRole('navigation', { name: 'Navegación principal' }).getByRole('link', { name: 'Ciudad', exact: true })).toBeInViewport()
  ...
  expect(await page.locator('#site-navigation').evaluate((node) => node.contains(document.activeElement))).toBe(false)
